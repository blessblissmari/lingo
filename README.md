...
> ⚠️ **status:** v0.2.3 — bootstrap interpreter, **working C backend**, **interactive REPL**, **owning vec + push/pop/set**, **traits + enum methods (static dispatch)**, **monomorphized `vec[Struct]` / `vec[Enum]`**, **`for ch in str:` UTF-8 codepoint iteration**, **`T ! E` + `?` + match-on-result** (including **`! str` raises with native parity** — `int(s) -> int!str` works on both backends), **`? else <expr>` error-type coercion** (v0.2.2 — lift `int!str` into a caller's `int!ParseErr` with `int(s)? else ParseErr.NotANumber`), **implicit `?` coercion via `impl From[E1] for E2:`** (new in v0.2.3 — drop the `else` annotation when a `From`-impl is in scope and plain `int(s)?` auto-wraps via the registered `from(e) -> E2`), **`Opt[T]` for `map.get(k)` with `match some(v): / none:` on both backends** (v0.2.1), **f-string interpolation of `struct` / `enum` / `Opt[T]` values**, **`str.trim()` + `vec[T].contains(x)`**, **`print(vec[T])` rendering vec contents**, **backwards element-type inference for `let mut x = vec[]`**, **C-keyword-safe local identifiers**, **`for _ in forever:` infinite loops**, **`let` shadowing diagnostics**, **full no-shadow parity across `let` / for-loop var / match-arm bind**, and **interp ≡ native parity for struct/enum debug, float formatting, `int(s)` error messages, and `Opt[T]` rendering**. 54/54 integration tests green; **29/31 applicable examples** produce byte-identical output across interp + native (all non-interactive examples match; the two skipped are interactive `io_roundtrip` + `fib_native_bench`).
