...
**`s.replace(from, to)` in native (v0.3.4)** — every non-overlapping
occurrence of `from` is substituted by `to`, with a single allocation
sized to the exact output length.  Non-empty `from` only (empty
