...
> ⚠️ **status:** v0.2.0 — bootstrap interpreter, **working C backend**, **interactive REPL**, **owning vec + push/pop/set**, **traits + enum methods (static dispatch)**, **monomorphized `vec[Struct]` / `vec[Enum]`**, **`for ch in str:` UTF-8 codepoint iteration**, **`T ! E` + `?` + match-on-result** (now including **`! str` raises with native parity** — `int(s) -> int!str` works on both backends), **f-string interpolation of `struct` / `enum` values**, **`str.trim()` + `vec[T].contains(x)`**, **`print(vec[T])` rendering vec contents**, **backwards element-type inference for `let mut x = vec[]`**, **C-keyword-safe local identifiers**, **`for _ in forever:` infinite loops**, **`let` shadowing diagnostics**, **full no-shadow parity across `let` / for-loop var / match-arm bind**, and **interp ≡ native parity for struct/enum debug, float formatting, and `int(s)` error messages** (the C backend's `lingo_str_debug_escape` mirrors Rust's `Debug for &str` byte-for-byte). 50/50 integration tests green; 25/29 applicable examples produce byte-identical output across interp + native (`tour` and `wordcount` remain interp-only, tracked for v0.2.1 — they still need `Opt[T]` / error-type coercion).
