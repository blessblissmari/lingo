...
> ⚠️ **status:** v0.2.7 — bootstrap interpreter, **working C backend**, **interactive REPL**, **owning vec + push/pop/set**, **traits + enum methods (static dispatch)**, **monomorphized `vec[Struct]` / `vec[Enum]`**, **`for ch in str:` UTF-8 codepoint iteration**, **`T ! E` + `?` + match-on-result** (including **`! str` raises with native parity** — `int(s) -> int!str` works on both backends), **`? else <expr>` error-type coercion** (v0.2.2 — lift `int!str` into a caller's `int!ParseErr` with `int(s)? else ParseErr.NotANumber`), **implicit `?` coercion via `impl From[E1] for E2:`** (v0.2.3 — drop the `else` annotation when a `From`-impl is in scope and plain `int(s)?` auto-wraps via the registered `from(e) -> E2`), **`float(s) -> float ! str` on both backends** (v0.2.4 — `lingo_float_parse` runtime helper mirrors `lingo_int_parse` byte-for-byte and slots into the same From-trait coercion so plain `float(s)?` propagates into a caller's `FloatErr` enum), **user-defined generic traits — `trait Foo[T1, T2, ...]:`** (v0.2.5 — bracket type-params declared once on the trait, supplied per-impl as `impl Foo[A1, A2] for Receiver:`, validated against a uniform arity gate; the built-in `From[E]` is now declared through the same machinery instead of parser magic, and the path is shared across user-defined and built-in generic traits), **trait method signature substitution** (new in v0.2.6 — `T` and `Self` in a trait method's declared signature get substituted with the impl's bracket args / target, then matched structurally against the impl method's signature; typos in impl method signatures are caught at resolve time with precise `parameter X expected A, got B` diagnostics on both backends), **`Opt[T]` for `map.get(k)` with `match some(v): / none:` on both backends** (v0.2.1), **f-string interpolation of `struct` / `enum` / `Opt[T]` values**, **`str.trim()` + `vec[T].contains(x)`**, **`print(vec[T])` rendering vec contents**, **backwards element-type inference for `let mut x = vec[]`**, **C-keyword-safe local identifiers**, **`for _ in forever:` infinite loops**, **`let` shadowing diagnostics**, **full no-shadow parity across `let` / for-loop var / match-arm bind**, and **interp ≡ native parity for struct/enum debug, float formatting, `int(s)` error messages, and `Opt[T]` rendering**. 65/65 integration tests green; **32/34 applicable examples** produce byte-identical output across interp + native (all non-interactive examples match; the two skipped are interactive `io_roundtrip` + `fib_native_bench`).
