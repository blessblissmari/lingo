...
> ⚠️ **status:** v0.2.1 — bootstrap interpreter, **working C backend**, **interactive REPL**, **owning vec + push/pop/set**, **traits + enum methods (static dispatch)**, **monomorphized `vec[Struct]` / `vec[Enum]`**, **`for ch in str:` UTF-8 codepoint iteration**, **`T ! E` + `?` + match-on-result** (including **`! str` raises with native parity** — `int(s) -> int!str` works on both backends), **`Opt[T]` for `map.get(k)` with `match some(v): / none:` on both backends** (new in v0.2.1 — `wordcount.lingo` now runs natively), **f-string interpolation of `struct` / `enum` / `Opt[T]` values**, **`str.trim()` + `vec[T].contains(x)`**, **`print(vec[T])` rendering vec contents**, **backwards element-type inference for `let mut x = vec[]`**, **C-keyword-safe local identifiers**, **`for _ in forever:` infinite loops**, **`let` shadowing diagnostics**, **full no-shadow parity across `let` / for-loop var / match-arm bind**, and **interp ≡ native parity for struct/enum debug, float formatting, `int(s)` error messages, and `Opt[T]` rendering**. 50/50 integration tests green; **26/29 applicable examples** produce byte-identical output across interp + native (`tour.lingo` remains interp-only, blocked on `?`-error-type coercion — tracked for v0.2.2).
