...
- [ ] **`int(s)` / `float(s)` parsing builtins in the C backend** — surfaced by the v0.1.29 audit (`tour.lingo` is currently interp-only). Land alongside a small `Opt[T]` story so `match` on `int(s)` and on `map.get()` works in native — see next item.
- [ ] **`match` on non-enum scrutinees in the C backend** — `wordcount.lingo` uses `match map.get(word):` which lowers fine in the interpreter but the C backend's `emit_match` only handles enum and `T!E` scrutinees. Either generalise via an `Opt[T]` monomorphization (like we did for `T!E` in v0.1.21) or fold this into the IR pass when SSA lands.
