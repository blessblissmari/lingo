...
- [ ] **`?` error-type coercion (v0.2.2)** — Currently `?` enforces `inner_e == raises.1` exactly; lift this so `int!str` flows into a function declared `_!ParseErr` via an explicit/implicit `From<str> for ParseErr` style mapping.  Closes the final `tour.lingo` gap and is a prereq for the layered error-handling story.  Either embed the coercion as syntactic sugar (`int(s)? else .ParseErr(...)`) or grow a real `from`-style trait.
- [ ] **`float(s)` parsing builtin in the C backend** — similar shape to `int(s)`; defer until we need it.
