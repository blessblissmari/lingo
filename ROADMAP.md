...
- [ ] **`Opt[T]` story (v0.2.1)** — `match` on `map.get(word):` for `wordcount.lingo` and the second half of `tour.lingo`. Either generalise via an `Opt[T]` monomorphization (same trick as `T!E` in v0.1.21) or fold into the IR pass when SSA lands. Also needs `?` to coerce from `int!str` into `int!ParseErr` (currently rejected — `?` enforces `inner_e == raises.1`) for the full `tour` to round-trip native.
